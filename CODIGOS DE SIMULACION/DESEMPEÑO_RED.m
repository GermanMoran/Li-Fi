 % Codigo realizado en matlab con la simulación de un canal
    ...
    M=16;
    S=log2(M);
    Rb = 0.092;                                      % Tasa de datos en Gbps
    %Rb = 0.138;
...
p1=0.6; % coeficiente de reflexion pared 1 (abajo)
p2=0;   % coeficiente de reflexion pared 2 (arriba)
p3=0.06;   % coeficiente de reflexion pared 3 (izquierda) 
p4=0; % coeficiente de reflexion pared 4 (derecha)
...
ancho=6.26; largo=6.47; alto=3.08;

h_tx_rx=alto-hpr;

Nx=ancho*10; Ny=largo*10; Nz=round(3*alto);
%dA=17.4e-3;
dA1=h_tx_rx*ancho/((Ny-1)*(Nz-1));
dA2=h_tx_rx*largo/((Nx-1)*(Nz-1));

%Construccion de la grilla

%x=linspace(0,ancho,Nx);
%y=linspace(0,largo,Nx);
        ...
        count=1;
            ...
            for k=1:length(y)
                ...
                index=find(round(tau1/delta_t)==t_vector);
                     ...
                     h_vector(index)=h_vector(index)+(m+1)*p1*Aef*dA2*(cos_phi^m)*(cos_alpha)*(cos_beta)*(cos_tetha1)/(2*pi^2*d1^2*d2^2);
                     end
                count=count+1;
                 end
            end
            ...
            for k=1:length(y)
                 ...
                 index=find(round(tau2/delta_t)==t_vector);
                     if abs(acosd(cos_tetha1))<=FOV
                     h_vector(index)=h_vector(index)+(m+1)*p2*Aef*dA2*(cos_phi^m)*(cos_alpha)*(cos_beta)*(cos_tetha1)/(2*pi^2*d1^2*d2^2);
                     end
                 count=count+1;
                 end
            end
            ...
            for k=1:length(x)
                 ...
                 index=find(round(tau3/delta_t)==t_vector);
                     if abs(acosd(cos_tetha1))<=FOV
                     h_vector(index)=h_vector(index)+(m+1)*Aef*p3*dA1*cos_phi^m*cos_alpha*cos_beta*cos_tetha1/(2*pi^2*d1^2*d2^2);
                     end
                 count=count+1;
                 end
            end
            %Reflection de la cuarta pared
             ...
             for k=1:length(x)
                 ...
                 index=find(round(tau4/delta_t)==t_vector);
                     if abs(acosd(cos_tetha1))<=FOV
                     h_vector(index)=h_vector(index)+(m+1)*p4*Aef*dA1*(cos_phi^m)*(cos_alpha)*(cos_beta)*(cos_tetha1)/(2*pi^2*d1^2*d2^2);
                     end
...
%% Canal de comunicaciones multitrayecto.
    ...
    % No se considera multitrayecto porque solo existe linea de vista en
...
%xlswrite('DCO_OFDM_RED_zona3', ber, 'Hoja1','A2');


semilogy(ebno_graf, ber_awgn, 'r-*', snrdb, ber,'g-*')
