...
%dA1=h*largo/(Ny*Nz);                         %Calculo diferencial de area
x=0:ancho/Nx:ancho;
y=0:largo/Ny:largo;
z=hpr:alto/Nz:alto;
...
figure (1)
Prx_dB1=10*log10((HLos)*Pled.*Ts.*G_Con); 
surfc(x,y,Prx_dB1),xlabel('X (m)'),ylabel('Y (m)'),zlabel('Potencia Recibida (dBm)'),box on ,colorbar


figure (2)
Prx_dB1=10*log10((Hnlos)*Pled.*Ts.*G_Con); 
surfc(x,y,Prx_dB1);
xlabel('X (m)');
ylabel('Y (m)');
...
figure (3)
Prx_dB1=10*log10((Htot)*Pled.*Ts.*G_Con); 
surfc(x,y,Prx_dB1);
xlabel('X (m)');
ylabel('Y (m)');
zlabel('Potencia Recibida (dBm)');
