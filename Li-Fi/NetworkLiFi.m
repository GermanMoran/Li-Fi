function [P_rec_A1,P_rec_A2,P_rec_A3,P_rec_A4,x,y]=NetworkLiFi(largo,ancho,alto,hpr,cr,cr2,cr3,cr4,flag1,flag2,xtx1,ytx1,xtx2,ytx2,xtx3,ytx3,xtx4,ytx4)
...
p1=cr;                                                             %Coeficiente de reflexion de todas las paredes
p2=cr2;                                                           %Coeficiente de reflexion de todas las paredes
p3=cr3; 
p4=cr4; 


% %Coeficiente de reflexión de c/d  Pared
% p1=0.6;                                                             %Coeficiente de reflexion de todas las paredes
        ...
        for k=1:length(y)
            for l=1:length(z)
               ...
               if abs(acosd(cos_tetha)) <=FOV  
                    H1(i,j)=H1(i,j)+((m+1)*p1*Aef*dA1*(cos_phi^m)*(cos_alpha)*(cos_beta)*(cos_tetha))/(2*pi^2*d1^2*d2^2);
 ...
 %        Pared 3
        ...
        for k=1:length(x)
            for l=1:length(z)
                 ...
                 if abs(acosd(cos_tetha)) <= FOV 
                    H3(i,j)=H3(i,j)+((m+1)*p3*Aef*dA1*(cos_phi^m)*(cos_alpha)*(cos_beta)*(cos_tetha))/(2*pi^2*d1^2*d2^2);
                 elseif abs(acosd(cos_tetha)) > FOV 
                      H3(i,j)=H3(i,j);
                  end
            end
        ...
        for k=1:length(x)
            for l=1:length(z)
                  ...
                  if abs(acosd(cos_tetha)) <= FOV 
                    H4(i,j)=H4(i,j)+((m+1)*p4*Aef*dA1*(cos_phi^m)*(cos_alpha)*(cos_beta)*(cos_tetha))/(2*pi^2*d1^2*d2^2);
...
if flag1==1 && flag2==0
    P_rec_A1=(HLos)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==0
    P_rec_A1=(H)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==1
...
H=H1+H2+H3+H4;
if flag1==1 && flag2==0 
    P_rec_A2=(HLos)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==0
    P_rec_A2=(H)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==1
...
if flag1==1 && flag2==0 
    P_rec_A3=(HLos)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==0
    P_rec_A3=(H)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==1
...
H=H1+H2+H3+H4;
if flag1==1 && flag2==0 
    P_rec_A4=(HLos)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==0
    P_rec_A4=(H)*P_total.*Ts.*G_Con;
elseif flag2==1 && flag1==1
