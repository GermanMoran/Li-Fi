...
function [Drms,x,y,t_vector,h_vector]=drms(largo,ancho,alto,p,hpr,posicion,a,PxTx,PyTx)
    
%ancho=5; largo=5; alto=3;   %Dimensiones del cuarto
     ...
     index=find(round(tau0/delta_t)==t_vector);
        ...
        count=1;
            ...
            %Reflection de la cuarta pared
             ...
             mean_delay=sum((h_vector).^2.*t_vector)./sum(h_vector.^2);
             Drms=sqrt(sum((t_vector-mean_delay).^2.*h_vector.^2)./sum(h_vector.^2));
             Rb=(1/(10*Drms));
             

                 axes(a)
                 stem(a,t_vector,h_vector,'.');
